...
* Added PhysicsNeMo-style directed
  :attr:`~isaaclab_neural.contacts.contact_set_schema.CONTACT_TOKEN_DIM`-channel contact token sets with
  pair-atomic packing, latent-query set encoding, regime/offline diagnostics, and dedicated Anymal native
  training configs.
* Added explicit world-frame and owner-identity metadata to contact-token datasets so incompatible caches
  are rejected.
