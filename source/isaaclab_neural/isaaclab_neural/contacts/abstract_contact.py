...
from isaaclab_neural.contacts.kernels import generate_contact_points_by_body_world
...
class AbstractContact:
    ...
    def create_abstract_contacts_buffer(self):
        # essential variables
        self.contact_shape0 = torch.empty((self.num_total_contacts,), dtype=torch.int32, device=self.device)
        self.contact_shape1 = torch.empty((self.num_total_contacts,), dtype=torch.int32, device=self.device)
        self.contact_point0 = torch.empty((self.num_total_contacts, 3), dtype=torch.float32, device=self.device)
        self.contact_point1 = torch.empty((self.num_total_contacts, 3), dtype=torch.float32, device=self.device)
        self.contact_normal = torch.empty((self.num_total_contacts, 3), dtype=torch.float32, device=self.device)
        self.contact_thickness0 = torch.empty((self.num_total_contacts,), dtype=torch.float32, device=self.device)
        self.contact_thickness1 = torch.empty((self.num_total_contacts,), dtype=torch.float32, device=self.device)

        #  derived variables (not used by newton default solvers)
        self.contact_depth = torch.empty((self.num_total_contacts,), dtype=torch.float32, device=self.device)
    ...
    def create_newton_contacts_view(self):
        ...
        contacts = Contacts(
            rigid_contact_max=self.num_total_contacts, soft_contact_max=0, requires_grad=False, device=warp_device
        )

        contacts.rigid_contact_count = wp.array([self.num_total_contacts], dtype=wp.int32, device=warp_device)

        contacts.rigid_contact_shape0 = wp.from_torch(self.contact_shape0)
        contacts.rigid_contact_point0 = wp.from_torch(self.contact_point0, dtype=wp.vec3)
        contacts.rigid_contact_shape1 = wp.from_torch(self.contact_shape1)
        contacts.rigid_contact_point1 = wp.from_torch(self.contact_point1, dtype=wp.vec3)
        contacts.rigid_contact_normal = wp.from_torch(self.contact_normal, dtype=wp.vec3)
        contacts.rigid_contact_depth = wp.from_torch(self.contact_depth)
        contacts.rigid_contact_thickness0 = wp.from_torch(self.contact_thickness0)
        contacts.rigid_contact_thickness1 = wp.from_torch(self.contact_thickness1)
    ...
    # Fill the contacts buffers with the values
    def initialize_contacts(self, model: newton.Model, newton_contacts: newton.Contacts):
        wp.launch(
