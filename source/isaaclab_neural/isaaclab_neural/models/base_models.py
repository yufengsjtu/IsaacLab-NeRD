...
class CNNBase(nn.Module):
    def __init__(self, input_shape, network_cfg, device='cuda:0'):
        ...
        num_layers = len(kernel_sizes)
        assert (
            len(layer_sizes) == num_layers and 
            len(stride_sizes) == num_layers and 
            len(padding_sizes) == num_layers
        )
        ...
        for i in range(len(kernel_sizes)):
            ...
            modules.append(
                model_utils.get_activation_func(network_cfg['activation'])
            )
            if network_cfg.get('layernorm', False):
                modules.append(nn.LayerNorm(layer_sizes[i]))
            in_channels = layer_sizes[i]
            out_shape = (
                out_shape + 2 * padding_sizes[i] - kernel_sizes[i]
            ) // (stride_sizes[i]) + 1
        ...
        if pool_cfg['name'] == 'MLP':
            ...
            modules.append(
                nn.Linear(
                    out_shape[0] * out_shape[1] * in_channels, 
                    out_features
                )
            )
            modules.append(
                model_utils.get_activation_func(pool_cfg['activation'])
            )
            self.out_features = out_features
        elif pool_cfg['name'] == 'SpatialSoftmax':
            modules.append(
                SpatialSoftArgmax(
                    normalize = pool_cfg.get('normalize', False)
                )
            )
