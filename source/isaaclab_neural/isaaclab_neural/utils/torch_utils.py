...
#### Set DETERMINISTIC_BEHAVIOR to True to disable torch.jit.script in this file to get deterministic behavior.
DETERMINISTIC_BEHAVIOR = True
if DETERMINISTIC_BEHAVIOR:
    torch.jit.script = lambda fn=None, *args, **kwargs: fn if fn is not None else lambda fn: fn


def to_torch(value, device):
    """Convert numeric data to a float32 tensor on the requested device."""
...
def quat_angle_diff(quat_0, quat_1):
    ...
    axis_norms = torch.clamp(torch.norm(quat_diffs[..., 0:3], 2, dim=-1), -1.0, 1.0)
...
"""
Reference: https://github.com/facebookresearch/pytorch3d/blob/main/pytorch3d/transforms/rotation_conversions.py#L498
"""
...
def exponential_coord_to_quat(exp_coord):
    ...
    return torch.cat([exp_coord * sin_half_angles_over_angles, torch.cos(angles * 0.5)], dim=-1)
...
@torch.jit.script
def convert_states_w2b(frame_pos, frame_quat, p, quat, lin_vel, ang_vel):
...
@torch.jit.script
def convert_states_b2w(frame_pos, frame_quat, p, quat, lin_vel, ang_vel):
