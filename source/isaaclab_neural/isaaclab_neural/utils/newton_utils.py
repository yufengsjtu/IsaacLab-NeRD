...
def base_joint_type(joint_types: np.ndarray) -> int | None:
...
def eval_ik(model, state):
    ...
    newton.eval_ik(model=model, state=state, joint_q=state.joint_q, joint_qd=state.joint_qd)
...
def assign_states_from_torch(newton_env, torch_states):
    ...
    wp.launch(
        ...
        inputs=[wp.from_torch(torch_states), newton_env.dof_q_per_env, newton_env.dof_qd_per_env],
        outputs=[newton_env.state.joint_q, newton_env.state.joint_qd],
